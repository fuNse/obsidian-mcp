...
import { createPromptsFromTools } from "./utils/tool-prompt-adapter.js";
...
async function main() {
  ...
  if (!vaultArgs || vaultArgs.length === 0) {
    const helpMessage = `
...
`;
  ...
  try {
    ...
    // Register generated prompts for all tools
    const generatedPrompts = createPromptsFromTools(tools);
    for (const prompt of generatedPrompts) {
      registerPrompt(prompt);
    }

    // Core prompts are registered in the server constructor
